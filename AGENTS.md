...
- 외부 에셋 다운로드 금지. 모든 텍스처·지오메트리는 절차 생성.
  **예외 2건**(둘 다 사용자 승인 2026-08-09):
  ① 타이틀 배경 `assets/title-bg.jpg`(AI 생성). 게임 월드에는 안 쓰이고 없으면 인게임 렌더로
  폴백한다. 지우지 마라 — 출처·프롬프트는 `docs/credits.md` §1.1(요강 필수 기재).
  ② **BGM** — 외부 라이선스 에셋 또는 AI 생성 트랙을 쓸 수 있다. 공포·추리 장르에서 음악이
  긴장 곡선 자체라는 판단(`docs/JUDGE-RUBRIC.md` J6). sfx·룸톤·리버브는 절차 생성을 유지한다.
  **라이선스 전문과 도구·프롬프트를 `docs/credits.md`에 반드시 기재**한다 — 요강 필수 요건이라
  누락하면 제출물 4번이 성립하지 않는다.
