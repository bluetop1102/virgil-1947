...
```
→ 깨끗한 인덱스에서 exit 0. **차단 경로 재현 필수** — 위반 1건(예: src/ 임시 파일에 Math.random( 직호출)을 git add 로 스테이지한 뒤 재실행하면 exit 1 + 위반 규칙명 출력, 그 파일을 git reset 으로 되돌리면 다시 exit 0. 깨끗한 인덱스 단독 실행은 차단 경로를 검증하지 못하므로 통과 근거가 아니다.

**A4.**

```bash
printf '#!/bin/sh\necho other\n' > .git/hooks/pre-commit && chmod +x .git/hooks/pre-commit && node tools/install-hooks.mjs
```
→ 남의 훅(마커 없음)은 덮어쓰지 않고 exit 1. 그 뒤 마커 있는 자기 산출 훅에 대해 재실행하면 멱등 갱신 exit 0 — 두 경로를 모두 재현한다
...
- E9 §2 판별 규칙 7줄과 다르게 구현하는 것 — 검사 범위 · 표출 판별(리터럴 전수 + lint-allow) · 훅 공존 · **훅 마커는 `# virgil-contract-lint managed` 고정 문자열 포함 여부로만 판별(변형 금지)** · `--staged` 수집은 `git diff --cached --name-only --diff-filter=ACM --no-renames` + 파일 전체 검사 · 허용 경로(atmosphere.js + atmo/** · materials/**) · 주석 제거 후 검사(단 lint-allow 는 제거 전 수집, 500줄만 원문 물리 줄 수)가 전부 계약에 정해져 있다. 추측으로 메우지 마라.
