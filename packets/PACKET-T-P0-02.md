# 패킷 T-P0-02 — 재질·조명 계약 예외 청소 — 팩토리 경유화
...
> 자동 생성물 — 손으로 고치지 않는다. 원천: `data/manifest/T-P0-02.json` · 생성기: `tools/packet-gen.mjs`

## 1. goal 계약

```
/goal 재질·조명 계약 예외 청소 — 팩토리 경유화 — T-P0-02
통과 조건: §8 수용 기준 3건 중 실행 가능한 전건 통과 · 콘솔 에러·경고 0 · §9 금지 사항 위반 0
...
- **타입**: level · **배정 모델**: codex · **상태**: in-progress
...
- `src/world/kit-mat.js` — 파일 전체
- `src/world/props.js` — **구역 한정** — 광원 생성 3건 (이 구역 밖은 남의 것이다)
- `src/world/testbed.js` — **구역 한정** — 광원 생성 1건 (이 구역 밖은 남의 것이다)
- `docs/ARCHITECTURE.md` — **구역 한정** — §6·§6.5 예외 등재 절 (이 구역 밖은 남의 것이다)
...
<!-- 원문: docs/ARCHITECTURE.md § 6.5 조명 계약 (ATMOSPHERE 소유, 레벨이 소비) ⊂ VIRGIL — 아키텍처 계약 v2 (모든 에이전트 필독) -->
...
### 4.5 RESUME §3.0 재질 복제 결함

*왜 읽는가: Material.copy()가 defines를 안 옮긴다 — 재질 복제는 반드시 kit-mat.cloneMat() 경유. 이 결함의 재발이 프로젝트 전역 스미어였다.*

<!-- 원문: docs/RESUME.md § 3.0 재질 오버레이 스미어의 진짜 원인 (해결됨 · 라운드5) ⊂ CECIL — 새 세션 인수인계 ⊂ 3. 기각된 가설 — 다시 파지 마라 -->
...
*왜 읽는가: 레벨이 소비하는 광원·재질 계약의 소비 측 규약.*

<!-- 원문: docs/design/E6-spaces.md § 2. 공통 계약 ⊂ E6 — 공간·레벨 -->
...
- `atmo-corridor-night` — **기존 엔트리, 수정 금지. 촬영만 한다.** 회귀 기준선 대조용 — 엔트리 수정 금지, 촬영만
  ```js
  'atmo-corridor-night': {
  ```
...
## 8. 수용 기준 (기계 판정 — 실행 가능한 전건 통과가 완료 조건)

형용사로 자평하지 않는다. 아래 명령을 실제로 돌리고 출력을 결과 보고에 붙인다.

**판정 3분류**: 각 항목은 **통과 · 미통과 · 대기** 중 하나로 보고한다. "도구 부재 시 건너뛴다"고
적힌 항목이 실제로 도구 부재로 실행 불가였다면 그것은 **대기**다 — 통과도 면제도 아니며, 회수 세션이
통합 시점에 실행해 최종 판정한다. 대기 항목이 있어도 §1 goal 계약의 통과 조건은 충족된다.

**착수 전 확인**: 자기 수용 기준에 `tools/lint-contract.mjs` 가 있다면 착수 전에
`node tools/lint-contract.mjs --staged` 로 자기 수정 대상 파일이 이미 통과하는지 본다.
기존 위반이 있으면 그것은 선행 결박이므로 §10.1 로 반환하지 말고 §10.2 보고에 적어 회수 세션에 넘긴다.
...
```bash
SHOT_PORT=5911 node tools/shoot.mjs --out shots/p0-02 atmo-corridor-night
```
→ 샷 1장 성공 · 콘솔 에러·경고 0
...
- 신규 셰이더·렌더 패스·재질 레시피 추가 — 그래픽 동결 구간이다.
- 재질 복제를 kit-mat.cloneMat() 밖 경로로 하는 것 — RESUME §3.0 결함의 재발이다.
- 픽셀이 바뀌는 리팩터 — pix diff 0.0%가 수용 기준이지 "더 나아 보임"이 아니다.
- props.js·testbed.js의 광원 생성 구역 밖 편집 — 그 파일의 나머지는 [PROPS] 소유다. props.js 는 T-P0-04 분할이 끝난 뒤 상태에서 작업한다(E10 §2 P0 의존).
...
- **주석·문자열 취급**: 패턴 규칙 4종(재질·광원·랜덤/시계·표출)은 줄 주석(`//`)·
  ...
  **원문 물리 줄 수** 기준.
- **패턴 규칙의 lint-allow (2026-08-07 신설 — #29 답신)**: `lint-allow: light-direct`
  (광원)·`lint-allow: material-direct`(재질) 주석을 허용하되, **ARCH 계약의 승인 예외
  절(§6.5 광원 4건 폐집합 등)에 등재된 좌표에서만 유효**하다 — 주석 단독으로는 예외가
  성립하지 않고(2중 결박), 린터는 규칙별 lint-allow 총수가 계약 등재 수를 초과하면
  FAIL 처리한다(현행 상한: light-direct 4 · material-direct 0 — 재질 예외는 아직 계약
  등재 0건, kit.js:324는 PROPS의 공급 측 정당성 소명 후 등재 여부 결정). 랜덤/시계
  규칙은 lint-allow 불허(결정론 계약에 예외 없음 — 범위 한정으로 이미 해소).
...
- **아트 디렉션 선언 (디렉터 확정 2026-08-07)**: 절차 휴머노이드는 포토리얼 인체가
  아니라 **의도된 퍼펫/마리오네트**(1947년 복화술 인형 계열)다 — 공포추리 톤과 정합하고,
  절차 생성 파이프라인의 실측 한계(포토리얼 도달 불가 — 품질 실험 3라운드 정체)를
  스타일 선언으로 전환한다. 완성도 기준: 관절·재질 일관성 · 마모 밀도 · 환경 정합
  (AAA-RUBRIC v3.1 D4 인물 예외·판정 기준). 의도된 인형 관절은 실격이 아니다.
  텔 시스템과의 정합: 페이셜 없음 계약(아래)·비얼굴 텔 채널(X6)은 마리오네트에서
  오히려 강화된다 — 인형의 몸이 말하는 것이 이 게임의 연기 문법이다.
...
### STORY §6 — 6. 환경 서사 (대사 없이 읽혀야 하는 것 — 루브릭 N6)

<!-- 원문: docs/STORY.md § 6. 환경 서사 (대사 없이 읽혀야 하는 것 — 루브릭 N6) ⊂ VIRGIL — 사건 성서 v2 (NARRATIVE / INTERROGATION 구현 명세) -->
...
### MASTER-PLAN §6 — 6. 루브릭 확장 — P축과 게이트 4 (`AAA-RUBRIC.md` v2 개정안)

<!-- 원문: docs/MASTER-PLAN.md § 6. 루브릭 확장 — P축과 게이트 4 (`AAA-RUBRIC.md` v2 개정안) ⊂ VIRGIL(가제) — 종합 기획 v1 (구 CECIL 확장) -->
