...
<!-- 원문: docs/design/E6-spaces.md § 로비 (1막) — "호텔의 얼굴, 은폐의 얼굴" ⊂ E6 — 공간·레벨 ⊂ 1. 공간별 사양 -->
...
<!-- 원문: docs/design/E2-golden-path.md § 1막 — 로비 (0:30–10:30, 10분) ⊂ E2 — 골든 패스 (최선 루트 완주 시나리오) ⊂ 완주 타임라인 (최선 루트, 50:00) -->
...
**신규 산출 티켓 주의**: "전체 스캔에 자기 파일 행 0건" 류 조건은 **파일을 만들지 않아도 문언상
통과한다**. 신규 소유 파일은 실재해야 완료다 — 보고에 `test -f <경로>` 출력을 함께 붙인다.
...
```
→ 전체 스캔 출력에서 src/world/lobby.js 행 0건 — 특히 atmosphere 밖 *Light 생성·materials 밖 재질 생성·500줄. 전체 스캔 종료 코드는 타 소유 파일의 기존 위반 때문에 1일 수 있다 — 그 경우 자기 소유 파일 행이 0건이면 통과이고, 잔여는 docs/HANDOFF.md 하단에 등재한다
