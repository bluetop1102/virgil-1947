...
- **2단 커널(E5 §6)**: 자유 서술 지목의 의미 매칭 대상은 `links[].claims` 정답 명제(및
  FACT 노드의 `text`)다. 명제가 문장 단위로 존재하는 것이 스왑의 전제 조건 — 이 그래프가
  곧 2단의 정답지다. 명제 후보(정답 1 + 오답 ≥2, 들러리 금지)는 factcheck J1~J3이
  기계 검증한다(E5 §5 지목판 개정, 2026-08-06).
...
```
→ FAIL 재현 — anxiousTell을 전부 제거해 거짓·텔 상관을 1.0으로 만든 변이에서 P5가 반드시 떨어진다(완전판별기 검출). 상관은 거짓 여부×텔 발화 여부의 이진 phi 계수로 정의하고, 분산 0 등 계산 불가 분포는 실패로 판정한다(구현 정본: tools/factcheck.mjs P5 절)
...
### E5 §6 — 6. 2단 커널 계약 (자유 서술 판정 — Phase 4 스왑)

<!-- 원문: docs/design/E5-interrogation.md § 6. 2단 커널 계약 (자유 서술 판정 — Phase 4 스왑) ⊂ E5 — 심문·판정 시스템 -->
