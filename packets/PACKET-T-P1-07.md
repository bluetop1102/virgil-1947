...
- **타입**: cinematic · **배정 모델**: opus · **상태**: done
...
```
→ 캡처 시퀀스가 E2 §첫 30초 표 5행과 초 단위로 대조 통과(P2) — 0:04 자막 개시, 0:10 페이드인, 0:22 배지, 0:27 조작권 이양·크로스헤어 페이드인 — tools/playthrough.mjs 는 T-P1-06 산출이라 이 클론에 없을 수 있다. 없으면 §10.1 반환이 아니라 이 항목을 '대기'로 보고에 표기하고(통과도 면제도 아니다) 부재 증명 출력을 붙인다. 나머지 실행 가능한 수용 기준은 전건 클론에서 실행한다
...
```
→ cinematic:start{id:"cin-intro"} → cinematic:end 발화 · 시퀀스 중 힌트 텍스트 0 · 종료 시점에 플레이어 조작권이 이양된다 — tools/playthrough.mjs 는 T-P1-06 산출이라 이 클론에 없을 수 있다. 없으면 §10.1 반환이 아니라 이 항목을 '대기'로 보고에 표기하고(통과도 면제도 아니다) 부재 증명 출력을 붙인다. 나머지 실행 가능한 수용 기준은 전건 클론에서 실행한다
...
```
→ 전체 스캔 출력에서 src/narrative/opening.js 행 0건 — 특히 Date.now()/performance.now() 직호출(타임라인은 engine.time). 전체 스캔 종료 코드는 타 소유 파일의 기존 위반 때문에 1일 수 있다 — 그 경우 자기 소유 파일 행이 0건이면 통과이고, 잔여는 docs/HANDOFF.md 하단에 등재한다
