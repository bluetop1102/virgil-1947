...
- `T-P1-09` — 심문 UI — 3선택 프롬프트 + 증거 지목 모드 · 상태 done
- `T-P1-10` — gameplay QA 하네스 — qa 구동·관측부 · 상태 done
...
```
→ **먼저 파일 존재를 검사한다** — 신규 산출 티켓에서 '전체 스캔에 자기 파일 행 0건'만 요구하면 파일을 만들지 않아도 통과하는 공허한 조건이 된다(T-P1-06 1차 반환 지적). tools/playthrough.mjs 가 실재하고, 전체 스캔 출력에서 그 행이 0건이어야 한다 — 특히 Date.now() 직호출(봇 시간도 결정론적 스텝). 전체 스캔 종료 코드는 타 소유 파일의 기존 위반 때문에 1일 수 있다 — 그 경우 자기 소유 파일 행이 0건이면 통과이고, 잔여는 docs/HANDOFF.md 하단에 등재한다
