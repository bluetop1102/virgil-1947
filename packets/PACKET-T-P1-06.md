...
```
→ 전체 스캔 출력에서 tools/playthrough.mjs 행 0건 — 특히 Date.now() 직호출(봇 시간도 결정론적 스텝). 전체 스캔 종료 코드는 타 소유 파일의 기존 위반 때문에 1일 수 있다 — 그 경우 자기 소유 파일 행이 0건이면 통과이고, 잔여는 docs/HANDOFF.md 하단에 등재한다
