...
// 저작 진술 id는 case-graph 정본 id다. 로더가 기존 저장 상태용 로컬 id와 관계 필드를 붙인다.
...
import { hydrateInterrogations, hydrateLinks } from './case-graph-loader.js'
  ...
  pryce: {
    ...
    statements: [
      ...
      },
      {
        ...
        onLieCorrect: { detective: '언니 부검 사본이 그 방에 있었습니다. 유족한테는 안 나가는 서류입니다. 나갈 수 있는 사람은 이 호텔에 하나뿐이고.', text: '…줬습니다. 급수 일지도 줬습니다. 그 아가씨가 그걸 들고 뭘 할지 알면서 줬습니다. 나는 늙었고 그 아가씨는 젊었습니다. 그게 다입니다. 그게 제가 한 겁니다.' },
        lieVariants: {
    ...
    outcomes: {
      full: { note: '4년 만에 944호의 커튼이 걷혔다. 전부 나왔다.' },
      partial: { note: '노인은 반쯤 고백했다. 나머지는 스크랩 뒤에 있다.' },
      fail: { note: '944호는 다시 14개월 전으로 돌아갔다.' }
    }
  },
...
export const INTERROGATIONS = hydrateInterrogations(INTERROGATION_SCRIPT)
...
export const LINKS = hydrateLinks(LINK_SCRIPT)
...
export const ENDINGS = {
  full: {
    ...
    subtitle: null,
    flagLines: {
      'deitch-confession': '다이치의 진술이 조서에 실렸다. 20달러도.',
      'doyle-pattern': '넬 밴스 파일에 루이즈의 증언이 첨부됐다.',
      'pryce-confession': '프라이스는 증인석에 앉겠다고 했다. 4년 만의 외출이다.'
    }
  },
  partial: {
    ...
    subtitle: null,
    flagLines: {
      'deitch-confession': '다이치는 진술을 반복했다. 변호사 앞에서는 반만.',
      'doyle-pattern': '루이즈의 증언은 48시간 뒤 철회됐다.',
      'pryce-confession': '프라이스의 진술서는 변호사 책상에서 멈췄다.'
    }
  },
  cold: {
    ...
    subtitle: '버질은 계속 영업했다. 9층 이야기가 하나 늘었다.',
    note: '미제 엔딩은 실패가 아니라 정식 엔딩이다. 재시도를 강요하지 않는다',
    flagLines: {
      'deitch-confession': '프런트의 진술은 조서에 남았다. 조서는 서랍에 남았다.',
      'doyle-pattern': '루이즈는 다시, 아무것도 못 들은 사람이 됐다.',
      'pryce-confession': '944호의 커튼은 다시 닫혔다.'
    }
  }
}
