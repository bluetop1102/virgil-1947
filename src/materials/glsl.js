// 절차 텍스처용 GLSL 스니펫 라이브러리.
// NOISE/PATTERN은 베이크 셰이더(raw GLSL3)와 재질 주입(GLSL1→ES3 승격) 양쪽에서 그대로 컴파일된다.
// SAMPLE은 texture2D/texture2DGradEXT를 쓰므로 재질 주입 전용이다.
// 모든 격자 노이즈는 vec2 주기를 받는다 — 베이크 결과가 이음매 없이 반복돼야 하기 때문.
...
// 재질 주입 전용 — 샘플링 계열. 여기서만 texture2D/texture2DGradEXT를 쓴다.
export function triplanarPatch (opts = {}) {
  const sharp = (opts.sharpness ?? 6.0).toFixed(1)
  return /* glsl */`
vec3 cTriBlend (vec3 n) {
  vec3 b = pow(abs(n), vec3(${sharp}));
...
export function stochasticTile (opts = {}) {
  const pw = (opts.blendPower ?? 7.0).toFixed(1)
  return /* glsl */`
...
export function pomPatch (steps = 24) {
  const n = Math.max(4, Math.min(64, steps | 0))
  return /* glsl */`
...
vec2 cPom (sampler2D hmap, vec2 uv, vec3 vts, float depth, float fade) {
  ...
  float layers = mix(float(${n}) * 0.4, float(${n}), clamp(vts.z, 0.0, 1.0));
  float dl = 1.0 / layers;
