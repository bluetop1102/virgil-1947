...
void SURF (vec2 uv, out vec3 alb, out float rgh, out float mtl, out float ao, out float alp) {
  ...
  'wood.painted.white': {
    seed: 31, mult: 0.4, bump: 2.6,
    // 도장 목재라도 그 밑은 나무다 — 결 방향으로 하이라이트가 끌리지 않으면 "매끈한
    // 그라디언트"로 읽힌다(심사 G6, 좌측 징두리). anisotropy 축은 uv.u = 결 방향이다.
    mat: { clearcoat: 0.25, clearcoatRoughness: 0.30, anisotropy: 0.38, anisotropyRotation: 0, envMapIntensity: 0.9 },
    // 복도 좌측 웨인스코트·마루 스트립은 화면의 큰 밝은 면이다. grungeScale 0.5(파장 2m)에
    // 진폭 0.17 로는 노출이 올라가면 통째로 크림색 한 장이 된다(G6 "넓은 단색 면").
    opts: { stochastic: true, grunge: 0.20, grungeScale: 0.66, detail: 0.30, detailTile: 10.0, ao: 0.5, toks: 1.2, damp: 0.36 },
    glsl: `
float pChip (vec2 uv) {
...
void SURF (vec2 uv, out vec3 alb, out float rgh, out float mtl, out float ao, out float alp) {
  ...
  'wallpaper.damask.green': {
    ...
    opts: { triplanar: 0.30, triSharp: 5, grunge: 0.36, grungeScale: 0.42, detail: 0.28, detailTile: 8.0, ao: 0.5, toks: 1.6, damp: 0.52, band: [0.42, 0.42, 2.95, 0.45] },
    glsl: `
...
void SURF (vec2 uv, out vec3 alb, out float rgh, out float mtl, out float ao, out float alp) {
  ...
  'carpet.corridor.red': {
    ...
    opts: { grunge: 0.34, grungeScale: 0.46, detail: 0.55, detailTile: 22.0, ao: 0.7, repeat: [3, 3], toks: 1.5, damp: 0.30 },
    glsl: `
