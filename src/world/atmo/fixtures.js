...
import { bevelBox, profile } from '../kit.js'
import {
  bulbGeo, stemGeo, galleryGeo, domeGeo, CHANNEL_PROF, REFLECTOR_PROF, fxHash
} from './fixture-geometry.js'
...
export function makeFixture (kind, col, hot) {
  ...
  if (kind === 'sconce') {
    ...
    if (col[2] > col[0] * 0.35) {
      const a = fxHash(col, hot, 4)
      const L = 0.60, hx = L * 0.5
      // 하우징은 재패닝한 강판이다. 아연도금(steel.galvanized)으로 뒀더니 밝은 천장을 배경으로
      // **더 밝은 청회색 평판**이 돼 기구가 아니라 판때기로 읽혔다 — 아래에서 올려다본 기구
      // 몸통은 밝은 천장에 대한 어두운 실루엣이어야 관이 광원으로 읽힌다(G1/G10).
      const body = mat('bakelite.black')
      // 채널 본체. 천장에 붙는 상단(y=0)에서 아래로 66mm.
      add(profile(CHANNEL_PROF, [[-hx, 0, 0], [hx, 0, 0]], { up: [0, 1, 0] }), body)
      // 엔드플레이트. 채널 양끝을 막아 관 끝이 허공에서 끊기지 않게 한다(G10 실루엣).
      for (const s of [-1, 1]) {
        add(bevelBox(0.010, 0.062, 0.160, 0.0025, 1), body, s * (hx - 0.005), -0.033, 0)
        // 텅스톤 소켓 — 관을 무는 물건이 보여야 관이 "떠 있는 막대"가 아니게 된다.
        // 몸통과 같은 검정이면 끝이 뭉개지므로 놋쇠로 값을 가른다.
        add(bevelBox(0.016, 0.034, 0.026, 0.004, 1), mat('brass.tarnished'),
          s * (hx - 0.018), -0.030, 0)
      }
      // 백색 에나멜 반사판. 그을음(a)으로 기구마다 탁도가 다르다.
      add(profile(REFLECTOR_PROF, [[-hx + 0.011, 0, 0], [hx - 0.011, 0, 0]], { up: [0, 1, 0] }),
        glass(0.22, 0.55 + a * 0.45))
      // 관은 반사판 아래로 내려와 열린 채널 밑면에서 그대로 보인다 — 화면에서 광원 위치를
      // 읽게 만드는 것이 이 기구의 본래 일이다(G1). 끝은 소켓이 문다.
      add(new THREE.CapsuleGeometry(0.019, L - 0.090, 6, 16), bulbMat(), 0, -0.049, 0)
        .rotation.z = Math.PI * 0.5
      // 천장 노크아웃 캐노피. 기구가 허공에 뜬 상자로 보이지 않게 매단 자리를 남긴다(D5)
      for (const dx of [-0.17, 0.17]) {
        add(bevelBox(0.070, 0.006, 0.070, 0.002, 1), mat('steel.galvanized'), dx, 0.004, 0)
      }
    } else {
