...
const BED_DEPTH = 0.42    // 룸톤 배율 0.58~1.42
const TICK_DT = 0.25      // 배율 갱신 간격(초). 매 프레임 쓰면 지퍼 노이즈가 난다
// 마루에서 7~14초, 골로 갈수록 30초까지 벌어지고 -0.15 아래에서는 아예 끊긴다. 배회 구간의
// 레벨을 실제로 움직이는 것은 연속층이 아니라 이 사건 밀도다(실측 — 파일 머리주석).
const FAR_BASE = 7
const FAR_SPAN = 7
const FAR_GATE = -0.15
...
const FAR = [
  ['pipe.knock', 0.50, 0.62, 2.6],      // 벽 속 배관이 한 번 튕긴다
  ['pipe.knock', 0.42, 0.80, 2.2],      // 같은 배관, 더 먼 마디
  // 아래 둘은 0.44·0.36 에서 바닥 위 8.7·10.0dB 로 솟았다(실측). 놀람의 문턱이라 내렸다 —
  // 목표대는 3~8dB, 즉 "저쪽에서 뭔가 났다"이지 "무언가 튀어나왔다"가 아니다.
  ['elevator.motor', 0.36, 0.56, 2.4],  // 케이블이 제 무게로 삐걱인다 — 부른 사람은 없다
  ['tank.hollow', 0.28, 0.74, 2.8]      // 위층 물탱크. 이 사건의 원점이 내는 소리다
]
...
function sched (a, t) {
  ...
  if (t > a.next.drip) {
    ...
    if (m.drip > 0.03) a.play('water.drip', { gain: m.drip * (0.35 + a.rand() * 0.5) * lift, rate: 0.8 + a.rand() * 0.45 })
  }
  if (t > a.next.tick) {
    a.next.tick = t + (6 + a.rand() * 16) * gap
    if (m.hum > 0.08) a.play('radiator.tick', { gain: (0.1 + a.rand() * 0.14) * lift, rate: 0.85 + a.rand() * 0.4 })
  }
  if (t > a.next.knock) {
