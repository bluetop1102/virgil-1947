...
// **배회 물방울은 제거됐다**(사용자 지시 2026-08-10 — "물소리도 그냥 없애버려라"). 간격·게인·
// 재생비를 두 차례 튜닝했는데도 실청취에서 계속 거슬린다는 판정이라, 튜닝이 아니라 큐 자체를
// 뺐다. 욕실 포함 전 공간이다. 수압 서사는 대사·증거(민원 장부·세면대 트랩)와 룸톤의 water 층이
// 계속 지므로 사실 그래프는 깨지지 않는다. 되살리지 마라 — 이 파일이 두 번 되돌아온 자리다.
// 인트로 시네마틱의 낙수 2발은 별개다(narrative/cinematics.js — 연출 소유, 배회 큐 아님).
