...
//
// **몸통(저역)에 `k:'t'` 를 쓰지 마라.** 발소리는 분당 100회 가까이 반복되는 최다 큐라, 한 음정에
// 몰린 저역이 있으면 그 주기가 그대로 북소리로 들린다(사용자 실청취 기각 2026-08-10 — carpet 의
// 88→52Hz 톤은 에너지의 99.9%가 120Hz 아래였고, tile 의 140Hz 톤은 스펙트럼 정점 자체였다).
// 저역은 반드시 `ft:'lp'` 광대역 노이즈로 낸다 — 같은 무게라도 퍼져 있으면 두께로 들리고
// 몰려 있으면 음정이 된다. 노이즈는 같은 g 라도 사인보다 에너지가 훨씬 적어 g 를 크게 잡는다.
export const FOOT = {
  marble: {
  ...
  tile: {
    dur: 0.44, peak: 0.8, layers: [
      ...
      { k: 'n', ft: 'lp', f: 520, d: 0.012, g: 0.55 },
...
export const SFX = {
  ...
  // 집는 타격(paper.pickup) 위에 겹치는 마찰층. 손가락이 섬유를 쓸며 나는 소리라 어택이 느리고
  // 알갱이가 여러 겹으로 어긋난다 — 타격 하나만으로는 종이가 아니라 딸깍으로 들린다.
  'paper.friction': {
    dur: 0.62, peak: 0.34,
    layers: [
      { k: 'n', ft: 'bp', f: 2400, q: 0.7, d: 0.12, g: 0.5, at: 0.035 },
      { k: 'n', ft: 'bp', f: 4300, q: 0.9, d: 0.07, g: 0.32, at: 0.02, t: 0.07 },
      { k: 'n', ft: 'hp', f: 5200, d: 0.05, g: 0.22, at: 0.012, t: 0.16 },
      { k: 'n', ft: 'bp', f: 1500, q: 1.4, d: 0.09, g: 0.26, at: 0.03, t: 0.24 },
      { k: 'n', ft: 'lp', f: 700, d: 0.06, g: 0.12, at: 0.02 }
    ]
  },
  'photo.flip': {
    dur: 0.4, peak: 0.5,
