// 건축 표면 레시피. 각 항목은 베이크 셰이더 조각(H/SURF)과 PBR 파라미터·타일링 파괴 옵션을 함께 들고 있다.
//   H(uv)     → 높이(0..1). sobel 미분으로 노멀이 유도되고 normalMap.a에 패킹돼 POM이 읽는다.
//   SURF(...) → albedo(linear) / roughness / metalness / ao / alpha
// mult는 quality.texRes 대비 해상도 배수, bump는 노멀 기울기 배율.
...
void SURF (vec2 uv, out vec3 alb, out float rgh, out float mtl, out float ao, out float alp) {
  ...
  'wood.varnished.dark': {
    ...
    opts: { grunge: 0.15, grungeScale: 0.45, detail: 0.35, detailTile: 9.0, ao: 0.55, toks: 1.2, damp: 0.20 },
    glsl: `
...
void SURF (vec2 uv, out vec3 alb, out float rgh, out float mtl, out float ao, out float alp) {
  ...
  'wood.painted.white': {
    seed: 31, mult: 0.25, bump: 2.0,
    mat: { clearcoat: 0.25, clearcoatRoughness: 0.30, envMapIntensity: 0.9 },
    opts: { stochastic: true, grunge: 0.17, grungeScale: 0.5, detail: 0.30, detailTile: 10.0, ao: 0.5, toks: 1.2, damp: 0.34 },
    glsl: `
float pChip (vec2 uv) {
...
float H (vec2 uv) {
  ...
  return 0.82 + brush * 0.10 - pChip(uv) * 0.55;
}
void SURF (vec2 uv, out vec3 alb, out float rgh, out float mtl, out float ao, out float alp) {
  float brush = FBM(uv, vec2(4.0, 46.0), 3);
  ...
  alb *= 1.0 - grime * 0.22;
  rgh = 0.30 + brush * 0.10 + chip * 0.48 + grime * 0.14;
  mtl = 0.0;
  ao = 1.0 - chip * 0.45;
  alp = 1.0;
}` },

  'wallpaper.damask.green': {
    seed: 47, mult: 1.0, bump: 3.7,
    mat: { sheen: 0.46, sheenRoughness: 0.36, sheenColor: 0x46604a, specularIntensity: 0.85, envMapIntensity: 0.80 },
    // 벽지는 메시 UV를 따르면 안 된다. 복도 벽 베니어는 uv 0..1 이 17.3m × 1.8m 를 덮어
    // 셀이 1.08m × 0.23m 로 찌그러지고, 그 결과 이 레시피 안의 얼룩·액자자국·이음매가 전부
    // 가로 줄무늬로 늘어나 화면에는 "변주 없는 격자"만 남는다 — D3 실격의 실제 원인이다.
    // 트리플래너로 월드 3.33m 타일을 깔면 어느 벽에서도 등방이고 셀은 33cm가 된다.
    // band: 하단 42cm 습기 + 몰딩(2.95m) 위 그을음. 월드 높이 기반이라 타일 위상과 어긋난다.
    //
    // grungeScale 0.115 는 파장 8.7m 라 복도 한 화면(약 4m) 안에서 상수였다 — 라지스케일
    // 그런지가 "있는데 안 보이는" 상태였고, 그게 D3 재지적의 실제 원인 중 하나다.
    // 0.42 면 주 성분이 2.4m / 0.84m 로 내려와 한 프레임 안에 얼룩 구획이 두세 개 들어온다.
    // 이 항은 월드 3D 노이즈라 텍스처 타일 위상과 무관하다 — 벽 높이가 타일의 45% 밖에
    // 안 되는 복도(가시 스트립이 uv.y 0.4~0.85로 고정)에서도 반드시 화면에 걸린다.
    opts: { triplanar: 0.30, triSharp: 5, grunge: 0.46, grungeScale: 0.42, detail: 0.28, detailTile: 8.0, ao: 0.5, toks: 1.6, damp: 0.58, band: [0.42, 0.42, 2.95, 0.45] },
    glsl: `
...
vec3 wDam (vec2 uv) {
  ...
  float a = (h.y - 0.5) * 0.175;              // 회전 ±5°
  float ca = cos(a), sa = sin(a);
  f = mat2(ca, -sa, sa, ca) * f * (1.0 + (h.x - 0.5) * 0.16);   // 스케일 ±8%
  f.x = abs(f.x);
  float v = floor(r * 3.0);
  float petals = 5.0 + v;
  ...
  // 셀 밝기 ±20%. 이게 없으면 모티프가 픽셀 단위로 동일한 사본으로 읽힌다(심사 D3 지적분)
  return vec3(m, 0.80 + 0.40 * q, smoothstep(0.42, 0.95, cHash12(id * 2.3 + 17.1)));
}
...
void SURF (vec2 uv, out vec3 alb, out float rgh, out float mtl, out float ao, out float alp) {
  vec3 dk = wDam(uv);
  float d = dk.x;
  vec3 ground = vec3(0.0102, 0.0198, 0.0132) * (0.84 + 0.32 * FBM(uv, vec2(9.0), 3));
  vec3 motif = vec3(0.0925, 0.1160, 0.0625) * dk.y;
  ...
  alb = mix(alb, vec3(0.0245, 0.0182, 0.0098), stain * 0.72);
  alb = mix(alb, vec3(0.0640, 0.0405, 0.0165), halo * 0.66);
  float drip = wDrip(uv);
  alb = mix(alb, vec3(0.0348, 0.0242, 0.0126), drip * 0.62);
  ...
  alb = mix(alb, vec3(0.0362, 0.0330, 0.0288), tr.x * 0.70);
  ...
  'plaster.cracked': {
    seed: 59, mult: 0.5, bump: 3.0,
    // 도장 석고는 순수 램버시안이 아니다 — 얇은 페인트 막이 광각 GGX 롭을 만든다.
    // clearcoat 없이 roughness만 0.86으로 두면 스펙큘러가 통계적으로 소멸해 알베도 색상표로 읽힌다(G3).
    mat: { clearcoat: 0.22, clearcoatRoughness: 0.40, specularIntensity: 1.0, envMapIntensity: 0.95 },
    opts: { triplanar: 0.62, grunge: 0.26, grungeScale: 0.26, detail: 0.42, detailTile: 6.0, ao: 0.75, toks: 1.25, damp: 0.42, band: [0.38, 0.40, 3.00, 0.45] },
    glsl: `
...
void SURF (vec2 uv, out vec3 alb, out float rgh, out float mtl, out float ao, out float alp) {
  ...
  'tile.hex.bathroom': {
    seed: 71, mult: 0.5, bump: 5.0,
    mat: { clearcoat: 0.55, clearcoatRoughness: 0.10, envMapIntensity: 1.05 },
    opts: { parallax: 0.030, grunge: 0.22, grungeScale: 0.40, detail: 0.22, detailTile: 12.0, ao: 0.8, repeat: [2, 2], toks: 1.2, damp: 0.48 },
    glsl: `
const float HXN = 7.0;
...
void SURF (vec2 uv, out vec3 alb, out float rgh, out float mtl, out float ao, out float alp) {
  ...
  'tile.subway.white': {
    seed: 83, mult: 0.5, bump: 5.5,
    mat: { clearcoat: 0.6, clearcoatRoughness: 0.09, envMapIntensity: 1.05 },
    opts: { parallax: 0.026, grunge: 0.21, grungeScale: 0.42, detail: 0.22, detailTile: 12.0, ao: 0.8, repeat: [2, 2], toks: 1.2, damp: 0.48 },
    glsl: `
...
void SURF (vec2 uv, out vec3 alb, out float rgh, out float mtl, out float ao, out float alp) {
  ...
  'concrete.rooftop': {
    seed: 97, mult: 0.5, bump: 2.6,
    mat: { envMapIntensity: 0.85 },
    opts: { triplanar: 0.45, grunge: 0.26, grungeScale: 0.20, detail: 0.40, detailTile: 5.0, ao: 0.7, toks: 2.0, damp: 0.45 },
    glsl: `
...
void SURF (vec2 uv, out vec3 alb, out float rgh, out float mtl, out float ao, out float alp) {
  ...
  'carpet.corridor.red': {
    seed: 103, mult: 0.5, bump: 3.4,
    // 울 카펫도 파일 끝단이 빛을 되쏜다 — sheen 없이 roughness 1.0 상수로 두면 스펙큘러가
    // 통계적으로 소멸해 알베도 색상표가 된다(G3). sheen 롭이 구 가장자리에 밝은 테를 만든다.
    // 파일은 한 방향으로 누워 있다 — 이방성 롭이 없으면 짜임이 아니라 벨루어 시트로 읽힌다(G6).
    mat: { sheen: 0.72, sheenRoughness: 0.42, sheenColor: 0x9c5a44, specularIntensity: 0.8, envMapIntensity: 0.55, anisotropy: 0.45, anisotropyRotation: 1.5708 },
    // grungeScale 0.24 는 파장 4.2m 라 러너 한 화면에서 거의 상수였다. 0.46 이면 1.5m / 0.55m
    // 성분이 살아나 통행 구역과 벽 쪽 죽은 구역이 화면 안에서 갈린다(G6 "단색 면" 지적분).
    opts: { grunge: 0.40, grungeScale: 0.46, detail: 0.55, detailTile: 22.0, ao: 0.7, repeat: [3, 3], toks: 1.5, damp: 0.34 },
    glsl: `
...
vec3 kFig (vec2 uv) {
  ...
  return vec3(clamp(max(max(lat, rim * 0.85), fleur * 0.9), 0.0, 1.0), ros, cid);
}
float H (vec2 uv) {
  vec3 k = kFig(uv);
  // 무늬 실은 바탕보다 굵어 실제로 도드라진다 — 스침각에서 이 요철이 무늬를 읽게 한다
  return 0.44 + kPile(uv) * 0.42 + k.x * 0.16 + k.y * 0.10 - kTrack(uv) * 0.30;
}
void SURF (vec2 uv, out vec3 alb, out float rgh, out float mtl, out float ao, out float alp) {
  vec3 k = kFig(uv);
  float cid = k.z;
  ...
  vec3 ground = vec3(0.0545, 0.0104, 0.0090) * (0.72 + 0.54 * cid);
  vec3 fig = vec3(0.2360, 0.1610, 0.0645) * (0.76 + 0.46 * cid);
  vec3 dark = vec3(0.0158, 0.0100, 0.0112) * (0.80 + 0.40 * cid);
  ...
  float soil = smoothstep(0.50, 0.86, FBM(uv, vec2(4.0), 4));
  alb *= 1.0 - soil * 0.34;
  ...
  mtl = 0.0;
  ao = 0.80 + 0.20 * pile - track * 0.12 - k.x * 0.06;
  alp = 1.0;
}` },
