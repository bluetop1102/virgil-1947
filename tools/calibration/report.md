...
> **경고 — 아래 Codex 결과는 구 패킷 실측이다.** 이 회차 이후 §3 의 결함 5건을 고쳐
> 패킷이 바뀌었다(§9·§11 중복 제거 · A3 수용 기준 교체 · 부록 2단 분리). 따라서
> **Grok 을 지금 돌려 아래와 비교하면 안 된다** — 서로 다른 지시서를 받은 두 에이전트를
> 비교하는 것이라 "에이전트가 갈렸는지"를 측정하지 못한다. 2에이전트 대조는 **둘 다
> 현재 패킷으로 재실행**해야 성립한다(`PROMPT-system-v2.md` §3). 아래 기록은 지우지 않는다 —
> 결함 수정이 무엇을 바꿨는지의 대조군이다.
