...
| 35:30 | 옥상 문 — 잠겨 있지 않다 | `act:enter {act:3}` |
...
발짝·룸톤 -6dB 캡처) · **클러치 검증** = C1~C5 각 좌표의 완주 봇 재현 캡처. 데모·트레일러가
...
- 완주 봇 구동 계약: `window.__CECIL__.qa` API(ARCHITECTURE v2 §9 — 타깃 슬러그는
  case-graph `obtain.where` 그대로) · 2모드: `--fast`(텔레포트, 도달성·이벤트 검증=P1) /
  `--paced`(실보행, 페이싱 실측=P3 ±40%·무사건 ≤3:00).
