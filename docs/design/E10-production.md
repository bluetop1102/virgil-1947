...
## 3. 모델 배정 원칙 (§7.3 승계 · **P1 운영 개정 — 디렉터 결정 2026-08-07**)

- **P1 운영(개정)**: Fable 토큰 예산 제약으로 "Claude 1회전 → 외부"를 폐기하고
  **codex 패킷 발주 우선**으로 전환한다 — 발주·회수·기계 게이트 검증·머지·블라인드
  채점은 발주 세션(Fable)이 집행하고, **Fable의 최종 체험 리뷰 1회를 후행**한다(픽셀·
  감각 판정의 최종 심급 유지). 발주 순서(의존 그래프 승계): 1파 T-P1-01·03·05 →
  2파 04·06·07·10 → 3파 08·09. T-P1-02는 마리오네트 정본 머지로 done. 운영 발사문:
  `PROMPT-system-v4.md`(발주 세션 소유 — 티켓 내용·수용 기준의 진실원은 이 보드 유지).
- **단일 최강 모델 고정(존치 — 시점만 이동)**: 그래픽 픽셀 축(동결 해제 후) · 공정
  오케스트레이션 · 채점 · 2단 판정 배터리 설계 · **P1 산출의 최종 체험 리뷰**(첫 30초·
  텔 타이밍·카메라 — 구현은 codex가 하되 승인은 감각 판정자가 한다).
- **외부 모델 투입 조건(개정)**: P0 완료(잔여: #29 답신 완료로 언블록) + 계약 문서와
  실코드의 정합. 티켓 양식: `{소유 파일(배타), 소비 계약(§ 인용), 수용 기준(기계
  게이트+샷/로그), 금지 사항, 반환 형식(CONTRACT_CHANGE_REQUEST 경로)}`.
  캘리브레이션 파일럿은 투입이 아니라 측정이다(산출물 비머지 드라이런 — 조건 무관).
