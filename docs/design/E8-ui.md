...
- **증거 지목 모드 (T-P1-09)**: 심문 중 LIE 선택 시 노트가 지목 모드로 열린다 —
  ...
  **"내민 증거는 무르지 못한다. 닫힌 진술은 다시 열리지 않는다. 수사는 계속된다."**
  (첫 문장 개정 2026-08-10 — 증거 아이템은 소모되지 않으므로 "회수되지 않는다"는 구현과
  어긋난다. 무를 수 없는 것은 제시 행위와 판정이다. SUBMISSION-AUDIT 항목의 게이트 판정.)
...
- **부트 순서**: 로딩(boot:progress) → 타이틀 → cin-intro 또는 체크포인트 복원.
  타이틀 소유: `ui/title.js` 신설(T-P1-05 범위) — 선택지는 놋쇠 명패.
- **타이틀 배경·조판(정본 — 2026-08-09 개정)**: 배경은 **비 내리는 1947 LA 아르데코 호텔
  외관**(`assets/title-bg.jpg`, AI 생성 — 제출물 4번에 도구·모델·프롬프트 기재). 이미지가
  없거나 `?titlebg=render` 면 **인게임 로비 실렌더**로 떨어진다(title.js 가 카메라를 잡고
  느린 드리프트를 준다. 로비는 부팅 때 이미 지어지므로 추가 로딩 비용 0). 글자만 남는
  화면은 어느 경로에서도 나오지 않는다. 조판은 호텔 간판 관습대로 작은 `HOTEL` / 큰
  `VIRGIL` / `1947` 세 단을 한 덩어리로 쌓는다 — 줄마다 top% 를 주면 화면비에 따라 글자
  상자가 겹친다. 스크림은 글자가 앉는 상·하단 띠만 눌러 대비를 벌고 중앙은 연다.
  구 사양("호텔 정문 유리 금박 각인")은 렌더 배경 경로에만 남는다.
- **제스처 게이트(정본 — 브라우저 자동재생·포인터록 요건)**: 무저장 회차의 타이틀은
  "자동 통과"가 아니라 **첫 입력 시 통과**다 — 로딩 완료 후 정지 화면에 놋쇠 벨 도상(SVG —
  금속 그라디언트·림 반사·누름버튼·2단 받침)과 그 아래 명패 **「체크인」** 한 장. 화면
  아무 곳이나 눌러도 통과한다(명패는 시선 유도). 첫 클릭/키 = 사용자 제스처로 AudioContext
...
- **재개 부트의 재제스처(정본)**: 리로드는 브라우저 사용자 활성화를 소멸시키므로,
  ...
  재입장 제스처 화면 한 장을 띄운다 — 타자기 두 줄 "돌아오셨습니까" /
  **"노트는 두고 가신 그대로입니다."** 아래에 §3 과 같은 벨 도상 + 명패 「이어서」.
  (둘째 줄은 복원 불변 — 소각·
...
걸린다. 고지문이 끝나면 비 내리는 호텔 외관 위로 금박 간판이 뜬다 — HOTEL VIRGIL, 1947.
그 아래 프런트 벨과 놋쇠 명패 「체크인」. 벨을 누르는 순간 물소리가 시작된다. 저장이 있으면
명패가 두 장으로 갈린다: 이어서, 처음부터. Tab을 누르면 게임이 멈추는 게 아니라
손이 노트를 든다. 심문에서 "거짓"을 고르면
노트가 지목 면으로 펼쳐지는데, 증거를 고르는 손 위로 면지의 활판 인쇄가 먼저 눈에
들어온다 — 내민 증거는 무르지 못한다. 닫힌 진술은 다시 열리지 않는다. 수사는
계속된다. 경고가 아니라 지급품에 원래 박혀 있던 문장이다. 타자로 정서된 진술 기록 사이에
...
- 배타 소유 파일: `docs/design/E8-ui.md` · (구현 시) `src/ui/notebook.js` `src/ui/settings.js`
  `src/ui/title.js` `src/ui/hud.js`(심문 3선택 프롬프트 절 — T-P1-09 원료)
  `src/ui/board.js`(증거판 제시 입력·주장란·서명 — §1·§4 소유 선언, T-P2-11 원료)
