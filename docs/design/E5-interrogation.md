...
옥상에서, 잘못 태운 것들의 빈자리가 판 위에 구멍으로 남아 있다. 증거 두 개를 자리에
놓고, 조서에 철된 가설 쪽지들을 넘긴다 — 이 둘이 말하는 게 무엇인지는 내가 고른다.
한 장을 뽑아 링크 자리에 끼운다. 도일이 웃는지, 웃음을 멈추는지 — 그것만이 답이다.
노트의 그어진 줄이 말한다: 이건 네가 태운 것이다.
