...
| 18 | 와이드가 밝은 건 오토노출 앵커 과다 (3차 "천장 최밝면") | 이중 앵커 하향 스윕 + 장면 광량 A/B (S-H) | **틀렸다** — e1(hiAnchor)이 바닥을 잡고, 광량을 내리면 자동노출이 되받아 95→115.6으로 오히려 밝아진다. 실결함은 **암부 부재**(천장 88.7 < 벽 102.7) — 자동노출이 못 되받는 비네트(톤매퍼 뒤 항)로 해소 |
| 19 | 램프 손 실루엣은 개구 회전·광속 하향으로 완화 | 셰이더 판독 + 회전·lumens A/B (S-H) | **무효** — `SHADE_FRAG`가 `abs(dot(N,V))`라 안팎 동일 발광이라 회전 무의미. lumens ×0.70에서 비 1.622→1.604뿐. 실효 레버는 `opts.hot`(발광 배수) 하나 — 투사광 미경유라 카메라 회피각 재검증도 불필요 |
| 20 | 코퍼 보 재질감은 이방성 복원+엣지 스펙큘러 AA 로 회복 | 7변종 A/B (S-H) | **기각** — 상쇄할 speckle 대가가 게이트 카메라에서 0(등재된 14.12는 자유배회 좌표), 복원은 근접 판독을 악화(청동→회색 슬래브), "직조 섬유" 판독은 절차 노멀에 구워진 소산이라 AA 무관(GLSL 재작성 사안) |
