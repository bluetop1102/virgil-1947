...
### P1-4. 라디오 디제틱 BGM — Kevin MacLeod로 확정 [2.5h]
**사용자 결정(2026-08-09): Kevin MacLeod(incompetech.com) CC-BY 4.0 트랙 사용 + 표기.**
AGENTS 예외 ②에 따라 이 트랙 파일 다운로드는 허용된다(BGM 한정).

- 배선: 외부 오디오 파일을 로비 라디오 버스에 태우는 경로(디코드→라디오 대역 필터·낡은
  스피커 착색→기존 라디오 게인·거리 감쇠·오답 -6dB 딥·심문 감쇠를 그대로 상속).
- 트랙 선정: incompetech.com에서 받아 라디오라이즈 상태로 2~3곡을 견주어라. 기준 —
  **느리고 쓸쓸한 인스트루멘털**(스윙 댄스곡은 톤 파괴), 1947 라디오에서 나올 법한 편성,
  루프 가능 길이. 시청 후보: "Night on the Docks"(색소폰 노어) · "I Knew a Guy"(뮤트
  트럼펫) · "Lobby Time"(라운지 비브라폰) · "Hard Boiled"(탐정 재즈 — 밝은 편이니 주의) ·
  "Backed Vibes Clean". 여러 곡을 쓸 수 있다 — 라디오 주파수 다이얼(기존 `radio.dial`
  상호작용)로 곡이 바뀌면 배치 자체가 연출이 된다.
- 최종 낙점은 사용자 귀 — 후보를 태운 빌드와 곡별 한 줄 소개를 완료 보고에 남겨라.
- `docs/credits.md` 기재(요강 필수): 곡명·작곡가·출처 URL·라이선스 전문 링크와 표준 표기 —
  `"곡명" Kevin MacLeod (incompetech.com) — Licensed under Creative Commons: By Attribution 4.0
  (creativecommons.org/licenses/by/4.0/)`. 인게임 크레딧 노출 경로가 있으면 HANDOFF로 제안.
