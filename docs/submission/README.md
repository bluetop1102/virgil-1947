...
| # | 제출물 | 형태 | 이 저장소의 재료 | 상태 |
|---|---|---|---|---|
| 1 | 플레이 가능한 빌드 + 소스 | GitHub 링크 | 배포·소스 모두 공개 완료 (§링크) | **성립** — P1 12/12 · 1막 완주 · 실배포 검증 PASS |
...
| 전체 소스 공개 | 성립 | 다운로드 에셋 0 · AI 생성 배경 1장(credits §1.1 기재) — 저장소가 곧 전체다 |
...
2. **게임이 1막을 완주한다 — P1 12장 전건 머지 (8/8 기준).**
   ...
   - **잔여(제출 차단 아님)**: ①`--paced` 실시간 모드가 막 전환에 도달하지 못한다(증거 획득은
     ...
     아직 빈 좌표를 찍는다.
   - **배포 갱신 완료(8/8)** — gh-pages `cf9153d` 가 P1 12장 반영본이다. 실배포 검증 PASS
     (부팅 성립·캔버스 1280×720·요청 실패 0·콘솔 0). 소스도 `main` 에 66커밋 push 완료.
