...
| # | 제출물 | 형태 | 이 저장소의 재료 | 상태 |
|---|---|---|---|---|
| 1 | 플레이 가능한 빌드 + 소스 | GitHub 링크 | 배포·소스 모두 공개 완료 (§링크) | **인프라 성립 · 콘텐츠 P1 7/10** |
...
| 커밋 기록 유지 | **성립**(공백 1건 명시) — §커밋 이력 | `git log` 112개(8/4~), 재작성 0 |
...
| 소스 | https://github.com/bluetop1102/virgil-1947 | **공개** — public, 112커밋 |
...
- 현재 112커밋 / 8-04 3, 8-05 22, 8-06 20, 8-07 이후 67 (P1 codex 발주 회수·머지 라운드).
...
2. **게임이 성립하지 않는다 — P1 진행 중(10장 중 7장 머지, 8/7 기준).**
   - **P0 종결.** 잔여였던 광원 4건은 8/7 `#29` 답신으로 **팩토리 경유 대신 ARCH §6.5 승인
     예외 등재**로 확정됐고(비등록 팩토리는 P5 이월), 린터에 2중 결박(등재 좌표 + 주석,
     수량 상한)을 구현해 닫았다. `props.js` 분할 커밋도 완료 — P0 6장 전건 종결.
   - **P1 머지분 7장**: 01 로비 정식 레벨 · 02 인물 리그(마리오네트 정본) · 03 다이치 텔
     연기 · 05 로딩·타이틀·설정·계측 · 07 첫 30초 시네마틱 · 10 gameplay QA 하네스.
     (02 는 8/7 이전 머지, 나머지 6장이 8/7 codex 발주 회수분)
   - **잔여 3장**: 04 심문 E2E(재발주 실행 중) → 06 완주 봇 · 08 심문 카메라 · 09 심문 UI.
     04 가 나머지 전부의 선행이라 임계경로다.
   - **미완의 실질**: 지금 링크를 눌러도 로비를 걷고 다이치를 보고 설정을 열 수는 있으나
     **심문이 성립하지 않아 사건을 풀 수 없다**. 04→06 이 닫혀야 "플레이 가능한 게임"이다.
