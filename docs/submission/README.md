...
1. **배포본 표출명 — 부분 해소.** `index.html` 의 `<title>`·부트 대문짝은 고쳤다(VIRGIL).
   **남은 3건은 인게임 문자열**이라 플레이 중에 보인다 — 수사노트 표지 "세실 호텔",
   미제 엔딩 캡션·자막 "세실은 계속 영업했다". `src/**` 는 이 세션의 편집 금지라
   `docs/HANDOFF.md` 등재분이고 소유자는 [UI]/[INTERROGATION]/[NARRATIVE] 다.
