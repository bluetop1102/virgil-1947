...
최종 배포·영상 업로드 뒤 아래 토큰 두 개를 실제 값으로 바꾸고 Form에 붙여 넣는다. `{{...}}`가
하나라도 남아 있으면 제출하지 않는다.

18:44 KST 로그인 상태에서 읽기 전용으로 재확인한 결과, 아래 두 링크 문항은 모두 필수 `textarea`이고
URL 전용 `pattern`이나 `maxlength`가 없다. 따라서 첫 문항에 플레이·소스·최종 커밋을 라벨과 함께
세 줄로 넣는다. 답변 입력·제출은 하지 않았다.

**플레이 가능한 빌드 + 전체 소스·커밋 이력** 입력란:

```text
플레이: https://bluetop1102.github.io/virgil-1947/
전체 소스·커밋 이력: https://github.com/bluetop1102/virgil-1947
최종 제출 커밋: https://github.com/bluetop1102/virgil-1947/commit/{{FINAL_COMMIT_SHA}}
```

**30~60초 실제 플레이 YouTube** 입력란:

```text
{{YOUTUBE_URL}}
```

파일 업로드란에는 각각 `HOTEL-VIRGIL-게임소개.pdf`와
`HOTEL-VIRGIL-AI활용기술.pdf`를 넣는다. `HOTEL-VIRGIL-PDF-MANIFEST.json`은 두 파일의 동결
증거로 로컬에 보관하되, Form에 별도 manifest 업로드란이 없으므로 임의로 다른 칸에 첨부하지 않는다.
...
2. **음원 귀속·가시성 이행** — 현재 후보의 라디오 3곡·긴장층 2곡은 모두 오디오 전용이고 true
   ...
   `measured_*`가 보존되지 않아 완전한 명령 재현성은 부분이다. `cbd02ad`는 이 한계를 재구성
   템플릿으로 명시하고 권리·AI 이력과 5곡 복사용 귀속 블록을 기술 PDF에 맞췄다. 남은 것은 헤드폰
   청감과 최종 Pages 재확인이다.
...
또한 실제 YouTube oEmbed, 깨끗하고 버전 관리된 입력, 격자문 3상태·증거 회귀 3종, 오디오 전용
스트림·설정 소스의 필수 귀속 문자열·true peak·지목판 이벤트 연결을 직접 검사한다. 설정 화면의
실제 가시성과 음원의 실제 청감은 자동화 범위가 아니므로 사람이 최종 배포에서 확인해 `설정 화면 육안 PASS`와
`지목 침대 런타임 청감 PASS`를 상태 인자로 남긴다. `output/`, `tmp/`, `dist/`, `shots/` 밖의
미추적 파일이 있으면 final을 거부한다.

AI 기술 원고의 `{{ELEVATOR_GATE_STATUS}}`는 사람이 상태 문자열을 입력하는 칸이 아니다. DRAFT에는
현재 후보에서 세 경로를 실행한 결과가 표시되고, final에서는 clean 동결 HEAD에서 다시 실행한 뒤 검증
HEAD와 PASS 문구를 자동으로 넣는다.
