...
**CECIL — 1947 · Room 942**
1947년 로스앤젤레스. LAPD 강력계 형사가 메인 스트리트의 호텔 세실에서 9일간 실종된 투숙객 아이리스 밴스(Iris Vance, 22)를 수사한다.
투숙객들의 수도에서 검은 물이 나오고, 압력이 떨어진다. 답은 옥상 물탱크에 있다.

- 실제 사건(엘리사 램 등)의 피해자를 직접 재현하지 않는다. 인물·사건은 전부 허구이며, 호텔의 시대·구조·도상만 차용한다.
- 톤: 필름 느와르 + 조용한 공포. 점프 스케어 금지. 공포는 공간·소리·정보의 공백에서 온다.
- 플레이 타임 목표 40~60분, 3막.
...
src/
  ...
  render/
    pipeline.js    [PIPELINE] 포스트프로세스 그래프
    passes/*.js    [PIPELINE] 개별 패스
  materials/
    procedural.js  [MATERIALS] GPU 텍스처 생성기
    library.js     [MATERIALS] 명명된 PBR 재질 레지스트리
  world/
    kit.js         [PROPS] 공유 지오메트리 키트(몰딩·베벨·가구)
    props.js       [PROPS] 소품 팩토리
    testbed.js     [PROPS] 재질·소품 쇼케이스 룸 (조기 시각 검증용)
    atmosphere.js  [ATMOSPHERE] IBL 환경·안개·볼류메트릭 파라미터·실광원 팩토리
    lobby.js       [LEVEL-LOBBY]
    corridor.js    [LEVEL-CORRIDOR]
    room942.js     [LEVEL-ROOM]
    rooftop.js     [LEVEL-ROOFTOP]
  chars/
    rig.js         [CHARACTERS] 절차적 휴머노이드 + 스키닝
    perf.js        [CHARACTERS] 미세신호(tell) 연기 시스템
  gameplay/
    player.js      [GAMEPLAY] 컨트롤러·카메라·상호작용 레이캐스트
    evidence.js    [GAMEPLAY] 증거 모델·수집
  narrative/
    script.js      [NARRATIVE] 전체 대사·스토리 데이터 (코드 없음, 데이터만)
    interrogation.js [INTERROGATION] 심문 상태기계
    deduction.js   [INTERROGATION] 증거판
    cinematics.js  [CINEMATICS] 카메라 시퀀스·타임라인
  audio/
